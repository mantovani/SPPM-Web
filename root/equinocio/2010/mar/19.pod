...
O cache é utilizado para sua aplicação ser escalavel, ou seja para manter resultados de querys, objetos que existe uma certa demora para ser calculado, thumbnails ou qualquer coisa que demore para ser gerado.

=item Tabela hash

Memcached se utiliza de uma gigante tabela hash distribuida em múltiplos servidores. Uma visão resumida de uma tabela hash caso você não saiba, ela é implementa uma "matriz (array) com elementos", no qual cada elemento contém uma lista de nós, como cada nó contendo chave e valor. 
...
Todo cache contem uma lista de slabs, que podemos dividir os blocos de memória em:
