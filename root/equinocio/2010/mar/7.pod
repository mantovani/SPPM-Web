...
O Captcha é uma ferramenta anti-spam/anti-bot, que nada mais é do que um teste
para saber se você é um ser humano ou não. Também pode ser usado contra
ataques de força bruta.

Uma maneira de saber que o usuário é de fato um usuário, é usando captcha na
autentificação, quando o cliente efetua o logon.
...
Existem técnicas para poder burlar o captcha, não é atoa que encontramos
imagens tortas e difíceis de ler. Até mesmo perguntas inteligentes, como:
"Qual letra não é vogal?","Qual número não é ímpar?" e etc. Imagens tortas,
testes sobre imagens dificultam muito o computador de entender, isso garante
ainda mais que não é um robô acessando sua aplicação.
...
digitar 2 palavras, uma delas o reCaptcha sabe qual é a outra não, ou seja,
você pega uma palavra que o OCR não conseguiu ler e diz para a máquina qual é
e ainda faz o teste de "humano". Mais informações aqui:
...
O Catalyst::Controller::reCAPTCHA precisa das chaves que você conseguiu se
cadastrando no site do reCaptcha como eu disse anteriormente, vamos fazer isso
setando dois atributos no MyCaptcha.pm. Adicione essas duas linhas B<com as
suas chaves>:
...
No primeiro arquivo, será enviado o captcha para um método que checa se está
certo ou não. O método que checa o captcha chama "checkcaptcha", veremos mais
sobre ele quando estivermos programando.
...
Agora no segundo template, colocaremos uma variável chamada "just_check", se o
captcha estiver correto ela será setada para "o usuário digitou certo" caso
contrário ela será setada para "o usuário digitou errado".
...
$c->stash->{recaptcha_ok} é atribuído para 1 se o usuário digitou o captcha
...
Usando o grande CPAN e escrevendo pouquíssimo código conseguimos implementar
um captcha.
