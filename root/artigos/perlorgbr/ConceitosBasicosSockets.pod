...
=head1 Conceitos bE<aacute>sicos sobre Sockets
...
=head2 3.4. Fechando a socket em caso de erro
   ...
   5                                    Proto => "tcp"   
   6                                   );
   7 die "Não foi possivel criar a socket\n" unless $socket;
...
=head2 4. Resultado final
   ...
   6                                    Proto => "tcp"   
   7                                   );
   8 die "Não foi possivel criar a socket\n" unless $socket;
   9 if ($socket) {
...
# 9148 bytes of input
#Mon Dec 14 02:15:40 2009 mantovani
